...
/*
 * pd-webserver modifications Copyright (c) 2021 Lucas Cordiviola
 * License http://opensource.org/licenses/mit-license.php MIT License
 */
...
lmain(t_webserver *x)
{
	...
	ctx = mg_start(&callbacks, 0, x->options);
	...
	for (n = 0; n < port_cnt && n < 32; n++) {
		...
		if ((ports[n].protocol & 1) == 1) {
			/* IPv4 */
			host = "localhost";
			logpost(x,2,"Browse files at %s://%s:%i/", proto, host, ports[n].port);
